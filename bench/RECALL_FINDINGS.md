...
| julia | JuliaIO/JSON.jl `src` | tree-sitter-julia 0.23.1 | 312 | 310 | 1.01x | 10.23x | 36.90x | Native tree-sitter AST walker preserves module/type/function/macro/constant coverage and precise callable-assignment handling, and now adds validated module/top-level variable assignments while excluding locals; expanded counter parity is 312/312 on JSON.jl, 1364/1364 on DataFrames.jl, and 605/605 on StatsBase.jl. |
| dart | dart-lang/http `pkgs/http/lib` | tree-sitter-dart 0.1.0 | 238 | 180 | 1.32x | 10.61x | 29.17x | Native tree-sitter AST walker preserves type/function/constructor/getter/setter/typedef coverage and now adds 58 validated top-level/class variable definitions while avoiding local variables and constructor-call false positives; expanded counter parity is 238/238 on http, 242/242 on path, and 203/203 on args. |
...
| verilog | lowRISC/ibex RTL slice | tree-sitter-systemverilog 0.3.1 module/package/function proxy | 144 | 93 | 1.55x | 10.62x | 29.83x | Native tree-sitter AST walker preserves the 93 module/package/function definitions and adds 51 validated SystemVerilog typedef/type definitions from the same RTL slice; the baseline parser marked 10 files as parse-error partial while still producing the matched module/function proxy. |
...
| powershell | PowerShell/PowerShellGet `src` | `pwsh` AST parser | 31 | 28 | 1.11x | 10.67x | 30.88x | Native tree-sitter-powershell route preserves function coverage and now adds validated module-scope variables while suppressing locals/property assignments; expanded `pwsh` AST validation is 31/31 on PowerShellGet, 626/632 recall with 0 extras on Pester, and 24/24 on ThreadJob. |
...
| Lang | Repo slice | Independent baseline | Atlas defs | Baseline defs | Recall/coverage | graphify latency | graphify tokens | Notes |
| --- | --- | --- | ---: | ---: | ---: | ---: | ---: | --- |
| sql | hasura/graphql-engine `server/src-rsr/migrations` | SQLFluff 3.5.0 core DDL + independent expanded DDL source counter | 131 | 111 | 1.18x | 10.22x | 30.63x | Native SQL DDL source parser preserves the SQLFluff table/view/function/procedure/trigger baseline and additionally indexes validated indexes, schemas, extensions, and temp-table declarations. Expanded DDL counts were checked on Hasura migrations, TimescaleDB pre-install SQL, and PostgreSQL contrib SQL slices; `.sql` files remain off the legacy fallback dispatcher. |
...
| ets/arkts | OpenHarmony TabsSample `entry/src/main/ets` | ets source counter | 161 | 153 | 1.05x | n/a | n/a | Native ArkTS/ETS source parser preserves type/function/method/constructor/variable coverage and now adds real ArkUI `@Component`/`@Entry` struct component symbols; expanded validation is 161/153 on TabsSample, 38/36 on codelabs, and 895/875 on applications_filepicker while graphify exposes no equivalent ETS query rows in this runtime (0/8). |
