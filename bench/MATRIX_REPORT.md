...
|---|---|---|
| go<br>`.go` via `extract_go` | ok | core matrix `go` ok; native scip-go:ok, gopls:ok; query eq 4/4, latency 6.0x, tokens 14.21x |
| python<br>`.py` via `extract_python` | ok | core matrix `python` ok; native scip-python:ok, pyright:ok; query eq 3/3, latency 5.97x, tokens 18.52x |
| javascript<br>`.js .jsx .mjs` via `extract_js` | ok | core matrix `javascript` ok; native scip-typescript:ok, tsserver:ok; query eq 3/4, latency 5.28x, tokens 7.0x |
| typescript<br>`.ts .tsx` via `extract_js` | ok | core matrix `typescript` ok; native scip-typescript:ok, tsserver:ok; query eq 3/4, latency 5.8x, tokens 11.42x |
| java<br>`.java` via `extract_java` | ok | core matrix `java` ok; native scip-java:missing, jdtls:ok; query eq 2/2, latency 3.82x, tokens 15.29x |
...
| c<br>`.c .h` via `extract_c` | ok | core matrix `c` ok; native clangd:ok; query eq 4/4, latency 5.96x, tokens 28.71x |
| cpp/cuda<br>`.cpp .cc .cxx .hpp .cu .cuh` via `extract_cpp` | ok | core matrix `cpp` ok; native clangd:ok; query eq 4/4, latency 5.68x, tokens 10.0x<br>`bench/LIVE_CUDA_SMOKE.json` ok; repo `https://github.com/NVIDIA/cuda-samples` commit `b7c5481c556c3fe98db060207ecaa41a4b9a9abc`; native `cuda-source-counter`; atlas_vs_cuda_source_counter_definition_ratio=1.0, query eq 3/3, latency 5.96x, tokens 6.67x |
...
|---|---|---|---|---|---|
| go | sirupsen/logrus | 679 symbols, 2102 calls, 0.362s cold full (0.033s delta) | 711 nodes, 333 calls, 0.619s full (0.277s delta) | 2225 symbols, 11887 occ, 0.246s | 12 pkgs, 0 diag, 0.377s |
| python | psf/requests | 517 symbols, 961 calls, 0.154s cold full (0.021s delta) | 580 nodes, 229 calls, 0.588s full (0.267s delta) | 1518 symbols, 8224 occ, 2.006s | 19 files, 12 diag, 0.736s |
| javascript | expressjs/express | 314 symbols, 435 calls, 0.097s cold full (0.026s delta) | 31 nodes, 3 calls, 0.137s full (0.133s delta) | 398 symbols, 2649 occ, 1.217s | 57 files, 257 diag, 0.538s |
| typescript | pmndrs/zustand | 227 symbols, 197 calls, 0.111s cold full (0.027s delta) | 112 nodes, 6 calls, 0.163s full (0.153s delta) | 792 symbols, 2461 occ, 1.065s | 124 files, 1 diag, 0.475s |
| java | google/gson | 1558 symbols, 3105 calls, 0.304s cold full (0.026s delta) | 1016 nodes, 927 calls, 0.863s full (0.552s delta) | missing | 54 doc syms, 425 diag, 1.88s |
| c | DaveGamble/cJSON | 1790 symbols, 4973 calls, 0.363s cold full (0.035s delta) | 971 nodes, 1018 calls, 0.858s full (0.47s delta) | n/a | 258 doc syms, 2 diag, 0.07s |
| cpp | google/leveldb | 2088 symbols, 9481 calls, 0.349s cold full (0.032s delta) | 2206 nodes, 1195 calls, 1.227s full (0.771s delta) | n/a | 421 doc syms, 167 diag, 0.22s |

## Derived Go ratios

- Build speed (cold-vs-cold, full index): Atlas COLD full index 0.362s vs graphify FULL extract 0.619s (graphify/Atlas = 1.71x); scip-go cold 0.246s (scip-go/Atlas = 0.68x); gopls (workspace type-check via `gopls stats`) cold 0.377s (gopls/Atlas = 1.04x). A ratio < 1.0x means Atlas is slower cold; this is the honest headline.
- Build speed (delta-vs-delta, no-change reindex): Atlas 0.033s vs graphify 0.277s, graphify/Atlas = 8.39x. Both tools re-run against an existing snapshot/sidecar here.
...
- gopls workspace truth: 12 workspace packages, 57 compiled Go files, 0 diagnostics, initial load 280.325ms.
- Query token cost (4/4 equivalent rows): graphify 398 tokens vs Atlas 28 tokens, graphify/Atlas = 14.21x.
- Query latency (4/4 equivalent rows): graphify 315.621ms vs Atlas 52.568ms, graphify/Atlas = 6.0x.
- Go cold-build saturation: cold-vs-cold full-index ratio is 1.71x (graphify FULL 0.619s / Atlas cold 0.362s), below 5x; Atlas's largest cold phases are build_symbols_edges:220ms, go_types:219ms, lexical:80ms.

## Derived Python ratios

- Build speed (cold-vs-cold, full index): Atlas COLD full index 0.154s vs graphify FULL extract 0.588s (graphify/Atlas = 3.82x); scip-python cold 2.006s (scip-python/Atlas = 13.03x). A ratio < 1.0x means Atlas is slower cold; this is the honest headline.
- Build speed (delta-vs-delta, no-change reindex): Atlas 0.021s vs graphify 0.267s, graphify/Atlas = 12.71x. Both tools re-run against an existing snapshot/sidecar here.
...
- Query latency (3/3 equivalent rows): graphify 241.532ms vs Atlas 40.475ms, graphify/Atlas = 5.97x.
- Python cold-build saturation: cold-vs-cold full-index ratio is 3.82x (graphify FULL 0.588s / Atlas cold 0.154s), below 5x; Atlas's largest cold phases are lexical:78ms, persist:78ms, write_sqlite:78ms.

## Derived JS/TS ratios

### javascript

- Build speed (cold-vs-cold, full index): Atlas COLD full index 0.097s vs graphify FULL extract 0.137s (graphify/Atlas = 1.41x); scip-typescript cold 1.217s (scip-typescript/Atlas = 12.55x). A ratio < 1.0x means Atlas is slower cold; this is the honest headline.
- Build speed (delta-vs-delta, no-change reindex): Atlas 0.026s vs graphify 0.133s, graphify/Atlas = 5.12x. Both tools re-run against an existing snapshot/sidecar here.
...
- TypeScript semantic check proxy: 57 files, 257 diagnostics, total 0.19s, memory 82426KB.
...
- Query latency (3/4 equivalent rows): graphify 220.133ms vs Atlas 41.698ms, graphify/Atlas = 5.28x.
- Query caveat: graphify missed 1 Atlas-selected hub symbols; raw rows remain in the table.
- javascript cold-build saturation: cold-vs-cold full-index ratio is 1.41x (graphify FULL 0.137s / Atlas cold 0.097s), below 5x; Atlas's largest cold phases are lexical:42ms, persist:42ms, write_sqlite:42ms.
### typescript

- Build speed (cold-vs-cold, full index): Atlas COLD full index 0.111s vs graphify FULL extract 0.163s (graphify/Atlas = 1.47x); scip-typescript cold 1.065s (scip-typescript/Atlas = 9.59x). A ratio < 1.0x means Atlas is slower cold; this is the honest headline.
- Build speed (delta-vs-delta, no-change reindex): Atlas 0.027s vs graphify 0.153s, graphify/Atlas = 5.67x. Both tools re-run against an existing snapshot/sidecar here.
- Atlas index phase timings: delta_check:0ms, resolve_head:0ms.
...
- TypeScript semantic check proxy: 124 files, 1 diagnostics, total 0.14s, memory 72442KB.
- LSP caveat: tsc returned diagnostics/exit 2; used as scriptable tsserver proxy.
- Query token cost (3/4 equivalent rows): graphify 217 tokens vs Atlas 19 tokens, graphify/Atlas = 11.42x.
- Query latency (3/4 equivalent rows): graphify 231.738ms vs Atlas 39.989ms, graphify/Atlas = 5.8x.
- Query caveat: graphify missed 1 Atlas-selected hub symbols; raw rows remain in the table.
- typescript cold-build saturation: cold-vs-cold full-index ratio is 1.47x (graphify FULL 0.163s / Atlas cold 0.111s), below 5x; Atlas's largest cold phases are lexical:28ms, persist:28ms, write_sqlite:28ms.

## Derived Java ratios

- Build speed (cold-vs-cold, full index): Atlas COLD full index 0.304s vs graphify FULL extract 0.863s (graphify/Atlas = 2.84x). A ratio < 1.0x means Atlas is slower cold; this is the honest headline.
- Build speed (delta-vs-delta, no-change reindex): Atlas 0.026s vs graphify 0.552s, graphify/Atlas = 21.23x. Both tools re-run against an existing snapshot/sidecar here.
...
- Query latency (2/2 equivalent rows): graphify 178.433ms vs Atlas 46.655ms, graphify/Atlas = 3.82x.
- Java cold-build saturation: cold-vs-cold full-index ratio is 2.84x (graphify FULL 0.863s / Atlas cold 0.304s), below 5x; Atlas's largest cold phases are lexical:190ms, persist:190ms, write_sqlite:190ms.

## Derived C/C++ ratios

### c

- Build speed (cold-vs-cold, full index): Atlas COLD full index 0.363s vs graphify FULL extract 0.858s (graphify/Atlas = 2.36x). A ratio < 1.0x means Atlas is slower cold; this is the honest headline.
- Build speed (delta-vs-delta, no-change reindex): Atlas 0.035s vs graphify 0.47s, graphify/Atlas = 13.43x. Both tools re-run against an existing snapshot/sidecar here.
...
- Query latency (4/4 equivalent rows): graphify 347.706ms vs Atlas 58.341ms, graphify/Atlas = 5.96x.
- c cold-build saturation: cold-vs-cold full-index ratio is 2.36x (graphify FULL 0.858s / Atlas cold 0.363s), below 5x; Atlas's largest cold phases are lexical:183ms, persist:183ms, write_sqlite:183ms.
### cpp

- Build speed (cold-vs-cold, full index): Atlas COLD full index 0.349s vs graphify FULL extract 1.227s (graphify/Atlas = 3.52x). A ratio < 1.0x means Atlas is slower cold; this is the honest headline.
- Build speed (delta-vs-delta, no-change reindex): Atlas 0.032s vs graphify 0.771s, graphify/Atlas = 24.09x. Both tools re-run against an existing snapshot/sidecar here.
...
- Query latency (4/4 equivalent rows): graphify 394.3ms vs Atlas 69.382ms, graphify/Atlas = 5.68x.
- cpp cold-build saturation: cold-vs-cold full-index ratio is 3.52x (graphify FULL 1.227s / Atlas cold 0.349s), below 5x; Atlas's largest cold phases are lexical:214ms, persist:214ms, write_sqlite:214ms.
...
|---|--:|--:|--:|--:|
| go | 0.337 | 1.264 | 13.178 | 10.43x |
| python | 0.504 | 1.577 | 13.479 | 8.55x |
| javascript | 0.505 | 1.085 | 13.668 | 12.6x |
| typescript | 0.468 | 0.929 | 13.668 | 14.71x |
| java | 0.54 | 11.192 | 23.328 | 2.08x |
| c | 0.504 | 2.095 | 14.346 | 6.85x |
| cpp | 0.499 | 2.404 | 14.798 | 6.16x |

- go warm-vs-warm context: both Atlas `serve` and gopls run as persistent daemons. Atlas warm explain median is 1.264ms and warm /healthz is 0.337ms. gopls's steady-state per-request latency is measured separately in its LSP smoke (different query semantics: a full Atlas context bundle vs a single LSP method), so the two are reported side by side, not as a single ratio.
- python warm-vs-warm context: both Atlas `serve` and pyright run as persistent daemons. Atlas warm explain median is 1.577ms and warm /healthz is 0.504ms. pyright's steady-state per-request latency is measured separately in its LSP smoke (different query semantics: a full Atlas context bundle vs a single LSP method), so the two are reported side by side, not as a single ratio.
- java warm-vs-warm context: both Atlas `serve` and jdtls run as persistent daemons. Atlas warm explain median is 11.192ms and warm /healthz is 0.54ms. jdtls's steady-state per-request latency is measured separately in its LSP smoke (different query semantics: a full Atlas context bundle vs a single LSP method), so the two are reported side by side, not as a single ratio.
- c warm-vs-warm context: both Atlas `serve` and clangd run as persistent daemons. Atlas warm explain median is 2.095ms and warm /healthz is 0.504ms. clangd's steady-state per-request latency is measured separately in its LSP smoke (different query semantics: a full Atlas context bundle vs a single LSP method), so the two are reported side by side, not as a single ratio.
- cpp warm-vs-warm context: both Atlas `serve` and clangd run as persistent daemons. Atlas warm explain median is 2.404ms and warm /healthz is 0.499ms. clangd's steady-state per-request latency is measured separately in its LSP smoke (different query semantics: a full Atlas context bundle vs a single LSP method), so the two are reported side by side, not as a single ratio.
...
|---|---|--:|--:|--:|--:|
| log | equivalent | 105 | 5 | 77.475 | 12.804 |
| newEntry | equivalent | 62 | 7 | 84.122 | 13.037 |
| releaseEntry | equivalent | 175 | 8 | 76.969 | 13.407 |
| Fire | equivalent | 56 | 8 | 77.055 | 13.32 |

### python

| Symbol | Status | graphify tokens | Atlas tokens | graphify ms | Atlas ms |
|---|---|--:|--:|--:|--:|
| get | equivalent | 142 | 6 | 81.07 | 13.418 |
| request | equivalent | 185 | 6 | 80.823 | 13.578 |
| __init__ | equivalent | 62 | 9 | 79.639 | 13.479 |

### javascript

| Symbol | Status | graphify tokens | Atlas tokens | graphify ms | Atlas ms |
|---|---|--:|--:|--:|--:|
| get | equivalent | 47 | 6 | 77.832 | 14.363 |
| sendFile | equivalent | 46 | 7 | 72.937 | 14.237 |
| defineGetter | equivalent | 47 | 7 | 69.364 | 13.098 |
| format | graphify_missing | 8 | 7 | 69.212 | 12.813 |

### typescript

| Symbol | Status | graphify tokens | Atlas tokens | graphify ms | Atlas ms |
|---|---|--:|--:|--:|--:|
| DevtoolsImpl | equivalent | 53 | 7 | 76.326 | 12.488 |
| hydrate | graphify_missing | 8 | 7 | 75.364 | 13.797 |
| shallow | equivalent | 75 | 6 | 76.313 | 13.962 |
| CreateStore | equivalent | 89 | 6 | 79.099 | 13.539 |

### java

| Symbol | Status | graphify tokens | Atlas tokens | graphify ms | Atlas ms |
|---|---|--:|--:|--:|--:|
| write | equivalent | 112 | 7 | 89.506 | 24.974 |
| read | equivalent | 102 | 7 | 88.927 | 21.681 |

### c

| Symbol | Status | graphify tokens | Atlas tokens | graphify ms | Atlas ms |
|---|---|--:|--:|--:|--:|
| cJSON_Delete | equivalent | 284 | 8 | 88.292 | 15.409 |
| cjson_functions_should_not_crash_with_null_pointers | equivalent | 288 | 18 | 86.529 | 14.28 |
| cJSON_CreateObject | equivalent | 332 | 9 | 86.883 | 14.413 |
| UnityPrint | equivalent | 302 | 7 | 86.002 | 14.239 |

### cpp

| Symbol | Status | graphify tokens | Atlas tokens | graphify ms | Atlas ms |
|---|---|--:|--:|--:|--:|
| RandomString | equivalent | 76 | 9 | 99.261 | 15.154 |
| MemEnvTest | equivalent | 100 | 8 | 98.209 | 13.465 |
| Size | equivalent | 72 | 9 | 99.168 | 14.442 |
| size | equivalent | 72 | 6 | 97.662 | 26.321 |
